  ...
  private final Setting<Integer> boostTicks =
      sgSpeed.add(
          new IntSetting.Builder()
              .name("Boost Ticks")
              .description(
                  "How many boosted speed packets should be sent before returning to normal speed.")
              .defaultValue(3)
              .min(1)
              .sliderMax(10)
              .visible(boost::get)
      ...
      if (hole != null && !singleBlocked()) {
        ...
        if (mc.player.getX() == hole.middle.x && mc.player.getZ() == hole.middle.z) {
          ...
          ((IVec3) event.movement)
              .meteor$setXZ(
                  Math.abs(x) < Math.abs(dX) ? x : dX, Math.abs(z) < Math.abs(dZ) ? z : dZ);
        }
      } else {
        this.toggle();
    ...
    for (int x = -range.get(); x <= range.get(); x++) {
      for (int y = -downRange.get(); y < 1; y++) {
        for (int z = -range.get(); z < range.get(); z++) {
          ...
          Hole hole =
              HoleUtils.getHole(
                  pos, singleHoles.get(), doubleHoles.get(), quadHoles.get(), depth.get(), true);
