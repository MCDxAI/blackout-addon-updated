...
public class JesusPlus extends BlackOutModule {
  ...
  private final Setting<Boolean> toggle =
      sgGeneral.add(new BoolSetting.Builder().name("Change speed").description("").build());
  ...
  public void onMove(PlayerMoveEvent event) {
    if (mc.player != null && mc.level != null) {
      if (mc.level.getBlockState(mc.player.blockPosition().below()).getBlock() == Blocks.WATER
          || mc.level.getBlockState(mc.player.blockPosition()).getBlock() == Blocks.WATER) {
        if (!inWater) {
          isSlowed = false;
        }
        inWater = true;
      ...
      if ((mc.player.isInWater() && !mc.player.isUnderWater())
          || (mc.player.isInLava() && !mc.player.isEyeInFluid(FluidTags.LAVA))) {
  ...
  private void OnRecieve(PacketEvent.Receive event) {
    if (event.packet instanceof ClientboundPlayerPositionPacket) isSlowed = true;
  }
