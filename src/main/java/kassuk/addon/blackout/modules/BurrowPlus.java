...
public class BurrowPlus extends BlackOutModule {
  ...
  private final Setting<Boolean> pFly =
      sgGeneral.add(
          new BoolSetting.Builder()
              ...
              .defaultValue(false)
              .build());
  private final Setting<Boolean> scaffold =
      sgGeneral.add(
          new BoolSetting.Builder()
              .name("Scaffold")
  ...
  public void onActivate() {
    ...
    InteractionHand hand =
        predicate.test(Managers.HOLDING.getStack())
    ...
    boolean rotated =
        instaRot.get()
            ...
            || Managers.ROTATION.startPitch(
                90, priority, RotationType.BlockPlace, Objects.hash(name + "placing"));
    if (!rotated) return;
    ...
    if (instaRot.get() && SettingUtils.shouldRotate(RotationType.BlockPlace)) {
      sendPacket(
          new ServerboundMovePlayerPacket.Rot(
              Managers.ROTATION.lastDir[0], 90, Managers.ON_GROUND.isOnGround(), false));
    }
    ...
    while (y < 1.1) {
      ...
      sendPacket(
          new ServerboundMovePlayerPacket.Pos(
              mc.player.getX(), mc.player.getY() + y, mc.player.getZ(), false, false));
    }
