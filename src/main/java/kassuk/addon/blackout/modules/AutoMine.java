...
public class AutoMine extends BlackOutModule {
  ...
  private final Setting<Priority> autoCityPriority =
      sgAntiSurround.add(
          new EnumSetting.Builder<Priority>()
              ...
              .description(
                  "Priority of anti surround. Places crystal next to enemy's surround block.")
              .defaultValue(Priority.Normal)
              .build());
  ...
  private void onRender(Render3DEvent event) {
    ...
    if (target != null) {
      if (lastState != null
          ...
          && !fastRemine.get()
          && !lastState.isSolid()
          && OLEPOSSUtils.solid2(target.pos)) {
        started = false;
      }
  ...
  private void render(Renderer3D r) {
    ...
    render =
        Mth.clamp(
            getMineTicks(slot, true) == getMineTicks(slot, false)
                ? render + delta * 2
                : render - delta * 2,
            -2,
            2);
    ...
    r.box(
        getRenderBox(p / 2),
        getColor(startColor.get(), endColor.get(), p, Mth.clamp(render, 0, 1)),
        getColor(lineStartColor.get(), lineEndColor.get(), p, Mth.clamp(render, 0, 1)),
        shapeMode.get(),
        0);
  ...
  private void update() {
    ...
    if (!started) {
      boolean rotated =
          !SettingUtils.startMineRot()
              || Managers.ROTATION.start(
                  target.pos, priority, RotationType.Mining, Objects.hash(name + "mining"));
  ...
  private void endMine() {
    ...
    sendSequenced(
        s ->
            new ServerboundPlayerActionPacket(
                ServerboundPlayerActionPacket.Action.STOP_DESTROY_BLOCK, target.pos, dir, s));
    ...
    switch (target.type) {
      case Cev, TrapCev, SurroundCev -> {
        ...
        if (!EntityUtils.intersectsWithEntity(
            AABB.of(new BoundingBox(target.crystalPos))
                .setMaxY(target.crystalPos.getY() + (SettingUtils.cc() ? 1 : 2)),
      ...
      case AutoCity -> {
        ...
        if (!EntityUtils.intersectsWithEntity(
            AABB.of(new BoundingBox(target.crystalPos))
                .setMaxY(target.crystalPos.getY() + (SettingUtils.cc() ? 1 : 2)),
            entity -> !entity.isSpectator())) return placeCrystal();
      }
      default -> {
        return true;
      }
  ...
  private Target getCev() {
    ...
    for (AbstractClientPlayer player : enemies) {
      ...
      if ((civ && pos.equals(civPos))
          && !(getBlock(pos) instanceof AirBlock)
      ...
      if (distanceCheck(civ, pos, distance, d)) {
        best =
            new Target(
                ...
                pos.above(),
                MineType.Cev,
                cevPriority.get().priority + (civ && pos.equals(civPos) ? 0.1 : 0),
                civ,
                false);
        distance = d;
      }
        ...
        if (distanceCheck(civ, pos, distance, d)) {
          best =
              new Target(
                  ...
                  pos.above(),
                  MineType.TrapCev,
                  trapCevPriority.get().priority + (civ && pos.equals(civPos) ? 0.1 : 0),
                  civ,
                  false);
          distance = d;
        }
        ...
        if (distanceCheck(civ, pos, distance, d)) {
          best =
              new Target(
                  ...
                  surroundCevPriority.get().priority + (civ && pos.equals(civPos) ? 0.1 : 0),
                  civ,
                  false);
          distance = d;
        }
  ...
  private Target getSurroundMiner() {
    ...
    for (AbstractClientPlayer player : enemies) {
      for (Direction dir : Direction.Plane.HORIZONTAL) {
        ...
        if (((!civ || !pos.equals(civPos)) && !OLEPOSSUtils.solid2(pos))
        ...
        if (distanceCheck(civ, pos, distance, d)) {
          best =
              new Target(
                  ...
                  surroundMinerPriority.get().priority + (civ && pos.equals(civPos) ? 0.1 : 0),
                  civ,
                  false);
          distance = d;
        }
        ...
        if (distanceCheck(civ, pos, distance, d)) {
          best =
              new Target(
                  ...
                  pos.relative(dir),
                  MineType.AutoCity,
                  autoCityPriority.get().priority + (civ && pos.equals(civPos) ? 0.1 : 0),
                  civ,
                  false);
          distance = d;
        }
  ...
  private Target getAntiBurrow() {
    ...
    for (AbstractClientPlayer player : enemies) {
      ...
      if (d < distance) {
        best =
            new Target(
                pos, null, MineType.AntiBurrow, antiBurrowPriority.get().priority, false, false);
        distance = d;
      }
    }
    return best;
    ...
    if (f == -1.0F) {
      ...
      float i =
          !state.requiresCorrectToolForDrops()
                  || mc.player.getInventory().getItem(slot).isCorrectToolForDrops(state)
              ? 30
              : 100;
  ...
  private float getSpeed(BlockState state, int slot, boolean speedMod) {
    ...
    if (effectCheck.get()) {
      ...
      if (mc.player.hasEffect(MobEffects.MINING_FATIGUE)) {
        f *=
            (float)
                Math.pow(0.3f, mc.player.getEffect(MobEffects.MINING_FATIGUE).getAmplifier() + 1);
      }
    }
    ...
    for (int i = 0; i < (pickAxeSwitchMode.get() == SwitchMode.Silent ? 9 : 35); i++) {
      if (slot == -1
          || (mc.player
                  .getInventory()
                  .getItem(i)
                  .getDestroySpeed(mc.level.getBlockState(target.pos))
              > mc.player
                  .getInventory()
                  .getItem(slot)
                  .getDestroySpeed(mc.level.getBlockState(target.pos)))) {
  ...
  private AABB getRenderBox(double progress) {
    return new AABB(
        target.pos.getX() + 0.5 - progress,
        target.pos.getY() + 0.5 - progress,
        target.pos.getZ() + 0.5 - progress,
        target.pos.getX() + 0.5 + progress,
        target.pos.getY() + 0.5 + progress,
        target.pos.getZ() + 0.5 + progress);
  }
  ...
  private boolean blocked(BlockPos pos) {
    ...
    return EntityUtils.intersectsWithEntity(
        box, entity -> entity instanceof Player && !entity.isSpectator());
  }
