...
public class PistonCrystal extends BlackOutModule {
  public PistonCrystal() {
    super(
        BlackOut.BLACKOUT,
        "Piston Crystal",
        "Pushes crystals into your enemies to deal massive damage.");
  }
  ...
  private void onRender(Render3DEvent event) {
    ...
    if (crystalPos != null) {
      event.renderer.box(
          getBox(crystalPos, crystalHeight.get()),
          crystalColor.get(),
          crystalLineColor.get(),
          crystalShapeMode.get(),
          0);
      event.renderer.box(
          getBox(pistonPos, pistonHeight.get()),
          pistonColor.get(),
          pistonLineColor.get(),
          pistonShapeMode.get(),
          0);
      event.renderer.box(
          getBox(redstonePos, redstoneHeight.get()),
          redstoneColor.get(),
          redstoneLineColor.get(),
          redstoneShapeMode.get(),
          0);
    }
  ...
  private void updateAttack() {
    ...
    for (Entity entity : mc.level.entitiesForRendering()) {
      ...
      if ((!alwaysAttack.get()
              && (c.getX() - c.getBlockX() == 0.5 && c.getZ() - c.getBlockZ() == 0.5))
          || !c.getBoundingBox()
              .intersects(AABB.of(new BoundingBox(crystalPos)).setMaxY(crystalPos.getY() + 1))) {
        continue;
      }
  ...
  private void updatePiston() {
    ...
    sendPacket(
        new ServerboundMovePlayerPacket.Rot(
            pistonDir.getOpposite().get2DDataValue(),
  ...
  private void updateCrystal() {
    ...
    if (EntityUtils.intersectsWithEntity(
        AABB.of(new BoundingBox(crystalPos)),
        entity -> !entity.isSpectator() && !(entity instanceof EndCrystal))) return;
    ...
    if (SettingUtils.shouldRotate(RotationType.Interact)
        && !Managers.ROTATION.start(
            crystalPos.below(), priority, RotationType.Interact, Objects.hash(name + "crystal")))
      return;
    ...
    if (hand == null) {
      switch (crystalSwitch.get()) {
        ...
        case PickSilent ->
            switched = BOInvUtils.pickSwitch(InvUtils.find(Items.END_CRYSTAL).slot());
  ...
  private void updateFire() {
    ...
    for (int x =
            (crystalDir.getOpposite().getStepX() == 0 ? -1 : Math.min(0, crystalDir.getStepX()));
        x
            <= (crystalDir.getOpposite().getStepX() == 0
                ? 1
                : Math.max(0, crystalDir.getOpposite().getStepX()));
        x++) {
      for (int y = 0; y <= 1; y++) {
        for (int z =
                (crystalDir.getOpposite().getStepZ() == 0
                    ? -1
                    : Math.min(0, crystalDir.getStepZ()));
            z
                <= (crystalDir.getOpposite().getStepZ() == 0
                    ? 1
                    : Math.max(0, crystalDir.getOpposite().getStepZ()));
            z++) {
          if (found) {
            break;
          }
    ...
    if (SettingUtils.shouldRotate(RotationType.BlockPlace)
        && !Managers.ROTATION.start(
            data.pos(), priority, RotationType.BlockPlace, Objects.hash(name + "fire"))) return;
    ...
    if (hand == null) {
      switch (fireSwitch.get()) {
        ...
        case PickSilent ->
            switched = BOInvUtils.pickSwitch(InvUtils.find(Items.FLINT_AND_STEEL).slot());
        case InvSwitch ->
            switched = BOInvUtils.invSwitch(InvUtils.find(Items.FLINT_AND_STEEL).slot());
      }
    }
  ...
  private void updatePos() {
    ...
    mc.level.players().stream()
        .filter(
            player ->
                player != mc.player
                    && player.position().distanceTo(mc.player.position()) < 10
                    && player.getHealth() > 0
                    && !Friends.get().isFriend(player)
                    && !player.isSpectator())
        .sorted(Comparator.comparingDouble(i -> i.position().distanceTo(mc.player.position())))
        .forEach(
            player -> {
              if (crystalPos == null) {
                update(player, true);
  ...
  private void update(Player player, boolean top) {
    ...
    for (Direction dir : Direction.Plane.HORIZONTAL) {
      ...
      if (SettingUtils.oldCrystals()
          && !(b instanceof AirBlock)
          && b != Blocks.PISTON_HEAD
          && b != Blocks.MOVING_PISTON) {
        continue;
      }
      if (mc.level.getBlockState(cPos.below()).getBlock() != Blocks.OBSIDIAN
          && mc.level.getBlockState(cPos.below()).getBlock() != Blocks.BEDROCK) {
        continue;
      }
      if (EntityUtils.intersectsWithEntity(
          AABB.of(new BoundingBox(cPos)).setMaxY(cPos.getY() + (SettingUtils.cc() ? 1 : 2)),
  ...
  private List<BlockPos> pistonBlocks(BlockPos pos, Direction dir) {
    ...
    for (int x = dir.getStepX() == 0 ? -1 : dir.getStepX();
        x <= (dir.getStepX() == 0 ? 1 : dir.getStepX());
        x++) {
      for (int z = dir.getStepZ() == 0 ? -1 : dir.getStepZ();
          z <= (dir.getStepZ() == 0 ? 1 : dir.getStepZ());
          z++) {
        for (int y = 0; y <= 1; y++) {
          if (x == 0 && y == 0 && z == 0
              || (SettingUtils.oldCrystals() && x == 0 && y == 1 && z == 0)) {
            continue;
          }
  ...
  private void redstonePos(BlockPos pos, Direction pDir, BlockPos cPos) {
    ...
    if (redstone.get() == Redstone.Torch) {
      for (Direction direction : Direction.values()) {
        ...
        if (!OLEPOSSUtils.replaceable(position)
            ...
            && !(mc.level.getBlockState(position).getBlock() instanceof FireBlock)) {
          continue;
        }
        ...
        if (!redstoneData.valid()
            ...
            || !SettingUtils.inMineRange(position)) continue;
    ...
    for (Direction direction : Direction.values()) {
      ...
      if (new AABB(position).intersects(OLEPOSSUtils.getCrystalBox(cPos))) {
        continue;
      }
      if (EntityUtils.intersectsWithEntity(
  ...
  private Entity crystalAt() {
    for (Entity entity :
        mc.level.getEntities(
            (Entity) null, new AABB(crystalPos), e -> e.blockPosition().equals(crystalPos))) {
      return entity;
    }
    return null;
  }
