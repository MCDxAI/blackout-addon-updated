...
public class HoleFillRewrite extends BlackOutModule {
  ...
  private void update() {
        ...
        InvUtils.findInHotbar(
            itemStack ->
                itemStack.getItem() instanceof BlockItem
                    && blocks.get().contains(((BlockItem) itemStack.getItem()).getBlock()));
    FindItemResult invResult =
        InvUtils.find(
            itemStack ->
                itemStack.getItem() instanceof BlockItem
                    && blocks.get().contains(((BlockItem) itemStack.getItem()).getBlock()));
    ...
    if (!placements.isEmpty()
      ...
      if (hand != null
          || (switchMode.get() == SwitchMode.Silent && result.slot() >= 0)
      ...
      for (int y = (int) -Math.ceil(range); y <= Math.ceil(range); y++) {
        for (int z = (int) -Math.ceil(range); z <= Math.ceil(range); z++) {
          ...
          for (BlockPos p : h.positions()) {
            ...
            if (d.valid()
                && closest >= 0
                && closest <= holeRange.get()
                && (!efficient.get()
                    || mc.player.position().distanceTo(Vec3.atCenterOf(p)) > closest)) {
