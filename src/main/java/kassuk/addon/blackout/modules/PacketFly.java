...
public class PacketFly extends BlackOutModule {
  ...
  private void onMove(PlayerMoveEvent e) {
    ...
    boolean shouldAntiKick =
        antiKick.get() && ticks % antiKickDelay.get() == 0 && !phasing && !onGround();
    ...
    for (; packetsToSend >= 1; packetsToSend -= 1) {
      ...
      offset =
          offset.add(
              strictVertical.get() && yOffset != 0 ? 0 : x,
              yOffset,
  ...
  private boolean onGround() {
    return mc.player.onGround()
        || (mc.player.getBlockY() - mc.player.getY() == 0
            && OLEPOSSUtils.collidable(mc.player.blockPosition().below()));
  }
  ...
  private Vec3 getBounds() {
    ...
    return new Vec3(
        Math.cos(Math.toRadians(yaw)) * xzBound.get(),
        yBound.get(),
  ...
  private void send(Vec3 pos, Vec3 bounds, boolean onGround) {
    ServerboundMovePlayerPacket.Pos normal =
        new ServerboundMovePlayerPacket.Pos(pos.x, pos.y, pos.y, onGround, false);
    ServerboundMovePlayerPacket.Pos bound =
        new ServerboundMovePlayerPacket.Pos(
            pos.x + bounds.y, pos.y + bounds.y, pos.z + bounds.z, onGround, false);
