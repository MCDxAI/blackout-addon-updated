...
public class AutoMend extends BlackOutModule {
  public AutoMend() {
    super(
        BlackOut.BLACKOUT, "Auto Mend", "Automatically mends your armor with experience bottles.");
  }
  ...
  private void onTick(TickEvent.Pre event) {
    ...
    if (timer >= 1) {
      InteractionHand hand =
          Managers.HOLDING.isHolding(Items.EXPERIENCE_BOTTLE)
              ? InteractionHand.MAIN_HAND
              : mc.player.getOffhandItem().getItem() == Items.EXPERIENCE_BOTTLE
      ...
      if (hand == null) {
        FindItemResult result =
            switchMode.get() == SwitchMode.PickSilent || switchMode.get() == SwitchMode.InvSwitch
                ? InvUtils.find(item -> item.getItem() == Items.EXPERIENCE_BOTTLE)
                : InvUtils.findInHotbar(item -> item.getItem() == Items.EXPERIENCE_BOTTLE);
      ...
      if (bottleSlot >= 0 && shouldThrow()) {
        shouldRot = true;
        boolean rotated =
            !(switchMode.get() == SwitchMode.Disabled && hand == null)
                && Managers.ROTATION.startPitch(
                    90, priority, RotationType.Use, Objects.hash(name + "look"));
