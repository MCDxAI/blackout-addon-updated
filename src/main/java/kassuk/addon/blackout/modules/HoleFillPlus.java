...
public class HoleFillPlus extends BlackOutModule {
  ...
  private final Setting<Boolean> near =
      sgNear.add(
          new BoolSetting.Builder().name("Near").description(".").defaultValue(true).build());
  ...
  private final Setting<Boolean> walking =
      sgWalking.add(
          new BoolSetting.Builder().name("Walking").description(".").defaultValue(true).build());
  ...
  private final Setting<Boolean> look =
      sgLooking.add(
          new BoolSetting.Builder().name("Looking").description(".").defaultValue(true).build());
  ...
  private final Setting<SurroundPlus.PlaceDelayMode> placeDelayMode =
      sgPlacing.add(
          new EnumSetting.Builder<SurroundPlus.PlaceDelayMode>()
    ...
    render.forEach(
        r -> {
          double progress =
              1
                  - Math.min(
                          System.currentTimeMillis() - r.time + renderTime.get() * 1000,
                          fadeTime.get() * 1000)
                      / (fadeTime.get() * 1000d);
  ...
  private void updateMaps() {
    ...
    ExtrapolationUtils.extrapolateMap(
        nearPosition,
        player -> player == mc.player ? selfExt.get() : nearExt.get(),
        player -> extSmooth.get());
    ExtrapolationUtils.extrapolateMap(
        boxes, player -> player == mc.player ? 0 : boxExt.get(), player -> boxExtSmooth.get());
  }

  private void updateWalk() {
    Map<AbstractClientPlayer, List<Movement>> newMap = new HashMap<>();
      ...
      Movement m =
          new Movement(
              Mth.wrapDegrees(
                  (float)
                          Math.toDegrees(
                              Math.atan2(player.getZ() - player.zo, player.getX() - player.xo))
                      - 90f),
              player.position());
  ...
  private void updateLook() {
    Map<AbstractClientPlayer, List<Look>> newMap = new HashMap<>();

    for (AbstractClientPlayer player : mc.level.players()) {
      Look e =
          new Look(Mth.wrapDegrees(player.getYRot()), player.getXRot(), player.getEyePosition());
  ...
  private void updateHoles() {
    ...
    int range =
        (int)
            Math.ceil(
                Math.max(SettingUtils.getPlaceRange(), SettingUtils.getPlaceWallsRange()) + 1);
    ...
    for (int x = -range; x <= range; x++) {
      for (int y = -range; y <= range; y++) {
        for (int z = -range; z <= range; z++) {
          ...
          if (!doubleHole.get() && (hole.type == HoleType.DoubleX || hole.type == HoleType.DoubleZ))
            continue;
  ...
  private boolean validPos(BlockPos pos) {
    ...
    return !BOEntityUtils.intersectsWithEntity(
        AABB.of(new BoundingBox(pos)),
  ...
  private boolean validHole(Hole hole) {
    double pDist =
        (nearPosition.containsKey(mc.player)
                ? feet(nearPosition.get(mc.player))
                : mc.player.position())
    ...
    for (AbstractClientPlayer player : mc.level.players()) {
      if (player.isSpectator()
          || player == mc.player
          || player.getHealth() <= 0
          || Friends.get().isFriend(player)) continue;
  ...
  private boolean selfCheck(Hole hole) {
    ...
    return selfWalking.get()
        && walkCheck(mc.player, hole, selfWalkMemory.get(), selfWalkingDist.get());
  }
  ...
  private boolean nearCheck(AbstractClientPlayer player, Hole hole, double pDist) {
    ...
    BlockPos pos =
        new BlockPos(player.getBlockX(), (int) Math.round(player.getY()), player.getBlockZ());
    ...
    double eDist =
        (nearPosition.containsKey(player) ? feet(nearPosition.get(player)) : player.position())
            .distanceTo(hole.middle);
  ...
  private boolean walkCheck(AbstractClientPlayer player, Hole hole, int ticks, double dist) {
    ...
    for (Movement m : walkAngles.get(player)) {
      ...
      double highestAngle =
          Mth.lerp(Math.min(player.position().distanceTo(hole.middle) / 8, 1), 90, 0);
  ...
  private boolean lookCheck(AbstractClientPlayer player, Hole hole) {
    ...
    for (Look l : lookAngles.get(player)) {
      ...
      double highestAngle =
          Mth.lerp(Math.min(player.position().distanceTo(hole.middle) / 20, 1), 35, 5);
  ...
  private void updatePlaces() {
    switch (placeDelayMode.get()) {
      ...
      case Seconds -> {
        if (placesLeft >= places.get()
            || System.currentTimeMillis() - lastTime >= placeDelayS.get() * 1000) {
          placesLeft = places.get();
  ...
  private void place(BlockPos pos) {
    ...
    if (SettingUtils.shouldRotate(RotationType.BlockPlace)
        && !Managers.ROTATION.start(
            data.pos(), priority, RotationType.BlockPlace, Objects.hash(name + "placing"))) return;
