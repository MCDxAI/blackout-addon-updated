...
public class ScaffoldPlus extends BlackOutModule {
  ...
  private final Setting<ScaffoldMode> scaffoldMode =
      sgGeneral.add(
          new EnumSetting.Builder<ScaffoldMode>()
              .name("Scaffold Mode")
  ...
  private void onTick(TickEvent.Pre event) {
    if (scaffoldMode.get() == ScaffoldMode.Legit) {
      mc.options.keyShift.setDown(
          mc.level.getBlockState(mc.player.blockPosition().below()).getBlock() instanceof AirBlock);
    }
  }
            ...
            if (hand == InteractionHand.OFF_HAND) {
              ...
              switch (switchMode.get()) {
                case Silent, Normal -> {
                  ...
                  block =
                      ((BlockItem) mc.player.getInventory().getItem(hotbar.slot()).getItem())
                          .getBlock();
                }
                case InvSwitch -> {
                  obsidian = BOInvUtils.invSwitch(inventory.slot()) ? inventory.count() : -1;
                  block =
                      ((BlockItem) mc.player.getInventory().getItem(inventory.slot()).getItem())
                          .getBlock();
                }
                case PickSilent -> {
  ...
  private AABB getBox(Vec3 vec) {
    ...
    return new AABB(
        vec.x - 0.3, vec.y, vec.z - 0.3, vec.x + 0.3, vec.y + (box.maxY - box.minY), vec.z + 0.3);
  }
