...
public class SpeedPlus extends BlackOutModule {
  ...
  private void onKB(PacketEvent.Receive event) {
    if (mc.player != null && mc.level != null) {
      if (knockBack.get()
          && event.packet instanceof ClientboundSetEntityMotionPacket packet
  ...
  public void onMove(PlayerMoveEvent event) {
    if (mc.player != null && mc.level != null) {
      ...
      if (mode.get() == SpeedMode.CCStrafe
          && (!onlyPressed.get() || strafeBind.get().isPressed())) {
        if (jumpPhase == 4) {
        ...
        case Accelerate -> {
          acceleration =
              Math.min(
                  ...
                  (move
                          ? acceleration
                              + (mc.player.onGround() || airStrafe.get()
                                  ? accelerationAmount.get() / 10
                                  : 0.02)
                          : acceleration)
                      * slipperiness(move));
          ...
          ((IVec3) event.movement)
              .meteor$setXZ(speed.get() * ax * acceleration, speed.get() * az * acceleration);
        }
      }
    }
  }
    ...
    return mc.player.onGround()
        ? mc.level
            .getBlockState(
                new BlockPos(
                    (int) mc.player.getX(),
                    (int) Math.ceil(mc.player.getY() - 1),
                    (int) mc.player.getZ()))
            .getBlock()
            .getFriction()
        : 0.98;
