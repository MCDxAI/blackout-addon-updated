...
public class StrictNoSlow extends BlackOutModule {
  public StrictNoSlow() {
    super(
        BlackOut.BLACKOUT,
        "Strict No Slow",
        "Should only be used on very strict servers. Requires any other noslow to work.");
  }
  ...
  public final Setting<Boolean> single =
      sgGeneral.add(
          new BoolSetting.Builder()
              .name("Single Packet")
              .description(
                  "Only sends 1 switch packet after starting to eat. Works on most servers that require this module.")
              .defaultValue(true)
              .build());
  ...
  private boolean shouldSend(ItemStack stack) {
    return mc.player != null
        && (onlyGap.get()
            || (stack != null && !stack.isEmpty() && stack.getItem() == Items.ENCHANTED_GOLDEN_APPLE
