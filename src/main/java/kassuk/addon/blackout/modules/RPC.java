...
/**
 * @author OLEPOSSU TrueFaxx: more options + safer cycling
 */
public class RPC extends BlackOutModule {
  ...
  private final Setting<String> appId =
      sgGeneral.add(
          new StringSetting.Builder()
              .name("app-id")
              .description(
                  "Discord application ID for the RPC. Needs a reload (toggle module) to apply.")
  ...
  private final Setting<String> menuDetails =
      sgText.add(
          new StringSetting.Builder()
              .name("menu-details")
  ...
  private final Setting<String> menuState =
      sgText.add(
          new StringSetting.Builder()
              .name("menu-state")
  ...
  private final Setting<CycleMode> cycleMode =
      sgBehavior.add(
          new EnumSetting.Builder<CycleMode>()
              .name("cycle-mode")
              .description("How the lines rotate.")
              .defaultValue(CycleMode.Sequential)
              .build());

  private final Setting<Boolean> syncLines =
      sgBehavior.add(
          new BoolSetting.Builder()
              .name("sync-lines")
  ...
  private final Setting<Boolean> cycleInMenu =
      sgBehavior.add(
          new BoolSetting.Builder()
              .name("cycle-in-menu")
              .description(
                  "If true, line cycling continues even in main menu (using your line lists).")
              .defaultValue(false)
              .build());

  private final Setting<Boolean> resetIndicesOnEnable =
      sgBehavior.add(
          new BoolSetting.Builder()
  ...
  // ---------- Images ----------
  private final Setting<Boolean> largeImageEnabled =
      sgImages.add(
          new BoolSetting.Builder()
  ...
  private final Setting<String> largeImageKey =
      sgImages.add(
          new StringSetting.Builder()
  ...
  private final Setting<String> largeImageText =
      sgImages.add(
          new StringSetting.Builder()
  ...
  private final Setting<Boolean> smallImageEnabled =
      sgImages.add(
          new BoolSetting.Builder()
  ...
  private final Setting<String> smallImageKey =
      sgImages.add(
          new StringSetting.Builder()
  ...
  private final Setting<String> smallImageText =
      sgImages.add(
          new StringSetting.Builder()
  ...
  private void updatePresence() {
    ...
    if (largeImageEnabled.get()) {
      presence.setLargeImage(
          largeImageKey.get().trim(), runStarscript(largeImageText.get(), "BlackOut"));
    } else {
      presence.setLargeImage(null, null);
    }

    if (smallImageEnabled.get()) {
      presence.setSmallImage(smallImageKey.get().trim(), runStarscript(smallImageText.get(), ""));
    } else {
