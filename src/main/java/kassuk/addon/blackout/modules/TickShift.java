...
public class TickShift extends BlackOutModule {
  public TickShift() {
    super(
        BlackOut.BLACKOUT,
        "Tick Shift",
        "Stores packets when standing still and uses them when you start moving.");
  }
  ...
  public final Setting<SmoothMode> smooth =
      sgGeneral.add(
          new EnumSetting.Builder<SmoothMode>()
              .name("Smoothness")
              .description(".")
  ...
  private void onMove(PlayerMoveEvent e) {
    if (e.movement.length() > 0
        && !(e.movement.length() > 0.0784 && e.movement.length() < 0.0785)) {
