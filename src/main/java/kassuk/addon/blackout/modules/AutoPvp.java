...
public class AutoPvp extends BlackOutModule {
  ...
  private final Setting<Integer> expAmount =
      sgSuicide.add(
          new IntSetting.Builder()
              .name("Exp Amount")
              .description(
                  "Suicides if there is under x amount of experience bottles in inventory.")
              .defaultValue(0)
              .min(0)
              .sliderRange(0, 256)
              .build());
  ...
  private void onRender(Render3DEvent event) {
    ...
    if (shouldSuicide) {
      if (!Modules.get().isActive(Suicide.class)) Modules.get().get(Suicide.class).toggle();
    } else {
      if (Modules.get().isActive(Suicide.class)) Modules.get().get(Suicide.class).toggle();
    }
    ...
    if (shouldSuicide) {
      if (baritone.get())
        BaritoneAPI.getProvider()
            .getPrimaryBaritone()
            .getCustomGoalProcess()
            .setGoalAndPath(new GoalRunAway(50, target.blockPosition()));
    ...
    if (!inRange && (mc.player.getY() > 100 || baritone.get())) {
      BaritoneAPI.getProvider()
          .getPrimaryBaritone()
          .getCustomGoalProcess()
          .setGoalAndPath(new GoalNear(target.blockPosition(), 3));
      path = null;
    } else {
      BaritoneAPI.getProvider().getPrimaryBaritone().getCustomGoalProcess().setGoalAndPath(null);

      if (!baritone.get()) path = RaksuTone.getPath(3, target.blockPosition());
    }
  }

  private void move(Vec3 movement, Vec3 vec) {
    MovementUtils.moveTowards(
        movement,
        0.2873,
        vec,
  ...
  private Predicate<ItemStack> getFood() {
    ...
    if (speedPotion.get()
        ...
        && !mc.player.hasEffect(MobEffects.SPEED)) return this::isSpeed;
  ...
  private boolean isSpeed(ItemStack stack) {
    for (MobEffectInstance instance :
      ...
      if (instance.getEffect() == MobEffects.SPEED) return true;
    }
    return false;
  }
  ...
  private boolean updateSuicide() {
    ...
    if (amountOf(i -> i.getItem() == Items.OBSIDIAN)
            + (eChests.get() ? amountOf(i -> i.getItem() == Items.ENDER_CHEST) * 8 : 0)
        <= obsidianAmount.get()) return true;
