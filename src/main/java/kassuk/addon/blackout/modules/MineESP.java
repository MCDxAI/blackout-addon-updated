...
public class MineESP extends BlackOutModule {
  ...
  private void onRender(Render3DEvent event) {
    ...
    renders.removeIf(
        r ->
            System.currentTimeMillis() > r.time + Math.round(maxTime.get() * 1000)
                || (render != null && r.id == render.id)
                || !OLEPOSSUtils.solid2(r.pos));
    ...
    renders.forEach(
        r -> {
          double delta =
              Math.min((System.currentTimeMillis() - r.time) / (maxTime.get() * 1000d), 1);
          event.renderer.box(
              getBox(r.pos, getProgress(Math.min(delta * 4, 1))),
              getColor(sideColor.get(), 1 - delta),
              getColor(lineColor.get(), 1 - delta),
              shapeMode.get(),
              0);
        });
  }
  ...
  private AABB getBox(BlockPos pos, double progress) {
    return new AABB(
        pos.getX() + 0.5 - progress / 2,
        pos.getY() + 0.5 - progress / 2,
        pos.getZ() + 0.5 - progress / 2,
        pos.getX() + 0.5 + progress / 2,
        pos.getY() + 0.5 + progress / 2,
        pos.getZ() + 0.5 + progress / 2);
  }
