...
public class AutoCrystalPlus extends BlackOutModule {
  ...
  private final Setting<DelayMode> placeDelayMode =
      sgPlace.add(
          new EnumSetting.Builder<DelayMode>()
  ...
  private final Setting<Double> expSpeed =
      sgExplode.add(
          new DoubleSetting.Builder()
              ...
              .sliderRange(0.01, 20)
              .build());
  private final Setting<Boolean> setDead =
      sgExplode.add(
          new BoolSetting.Builder()
              .name("Set Dead")
              .description(
              ...
              .defaultValue(false)
              .build());
  private final Setting<Double> setDeadDelay =
      sgExplode.add(
          new DoubleSetting.Builder()
              .name("Set Dead Delay")
  ...
  private final Setting<DmgCheckMode> dmgCheckMode =
      sgDamage.add(
          new EnumSetting.Builder<DmgCheckMode>()
              .name("Dmg Check Mode")
  ...
  private final Setting<ExplodeMode> expMode =
      sgDamage.add(
          new EnumSetting.Builder<ExplodeMode>()
  ...
  private final Setting<Integer> extrapolation =
      sgExtrapolation.add(
          new IntSetting.Builder()
              .name("Extrapolation")
              .description(
                  "How many ticks of movement should be predicted for enemy damage checks.")
              .defaultValue(0)
              .range(0, 100)
              .sliderMax(20)
              .build());
  private final Setting<Integer> rangeExtrapolation =
      sgExtrapolation.add(
          new IntSetting.Builder()
  ...
  private final Setting<EarthFadeMode> earthFadeMode =
      sgRender.add(
          new EnumSetting.Builder<EarthFadeMode>()
  ...
  // --------------------Compatibility--------------------//
  private final Setting<Double> autoMineDamage =
      sgCompatibility.add(
          new DoubleSetting.Builder()
  ...
  private final Setting<Boolean> amPlace =
      sgCompatibility.add(
          new BoolSetting.Builder()
              ...
              .defaultValue(true)
              .build());
  private final Setting<Double> amProgress =
      sgCompatibility.add(
          new DoubleSetting.Builder()
              ...
              .visible(amPlace::get)
              .build());
  private final Setting<Boolean> amSpam =
      sgCompatibility.add(
          new BoolSetting.Builder()
              .name("Auto Mine Spam")
  ...
  private final Setting<AutoMineBrokenMode> amBroken =
      sgCompatibility.add(
          new EnumSetting.Builder<AutoMineBrokenMode>()
  ...
  private void onTickPost(TickEvent.Post event) {
    ...
    ExtrapolationUtils.extrapolateMap(
        extPos,
        player -> player == mc.player ? selfExt.get() : extrapolation.get(),
        player -> extSmoothness.get());
    ExtrapolationUtils.extrapolateMap(
        extHitbox, player -> hitboxExtrapolation.get(), player -> extSmoothness.get());

    AABB rangeBox =
        ExtrapolationUtils.extrapolate(mc.player, rangeExtrapolation.get(), extSmoothness.get());
    if (rangeBox == null) rangePos = mc.player.getEyePosition();
    else
      rangePos =
          new Vec3(
              (rangeBox.minX + rangeBox.maxX) / 2f,
    ...
    existedList.forEach(
        (key, val) -> {
          if (System.currentTimeMillis() - val >= 5000 + existed.get() * 1000) toRemove.add(key);
        });
    toRemove.forEach(existedList::remove);

    toRemove.clear();
    existedTicksList.forEach(
        (key, val) -> {
          if (ticksEnabled - val >= 100 + existedTicks.get()) toRemove.add(key);
        });
    ...
    own.forEach(
        (key, val) -> {
          if (System.currentTimeMillis() - val >= 5000) toRemove.add(key);
        });
        ...
        case BlackOut -> {
          ...
          if (renderPos != null) {
            ...
            if (r >= 0.001) {
              ...
              AABB box =
                  new AABB(
                      renderPos.x() + 0.5 - width,
                      renderPos.y() + down,
                      renderPos.z() + 0.5 - width,
                      renderPos.x() + 0.5 + width,
                      renderPos.y() + up,
                      renderPos.z() + 0.5 + width);
              ...
              event.renderer.box(
                  box,
                  new Color(color.get().r, color.get().g, color.get().b, color.get().a),
                  lineColor.get(),
                  shapeMode.get(),
                  0);
            }
          ...
          if (renderProgress > 0 && renderPos != null) {
            event.renderer.box(
                ...
                new Color(
                    ...
                    color.get().g,
                    color.get().b,
                    (int) Math.round(color.get().a * Math.min(1, renderProgress / fadeTime.get()))),
                new Color(
                    lineColor.get().r,
                    lineColor.get().g,
                    lineColor.get().b,
                    (int)
                        Math.round(
                            lineColor.get().a * Math.min(1, renderProgress / fadeTime.get()))),
                shapeMode.get(),
                0);
          }
        }
            ...
            if (alpha[0] <= delta) {
              ...
              event.renderer.box(
                  ...
                  new Color(
                      ...
                      color.get().g,
                      color.get().b,
                      (int) Math.round(color.get().a * Math.min(1, alpha[0] / alpha[1]))),
                  new Color(
                      lineColor.get().r,
                      lineColor.get().g,
                      lineColor.get().b,
                      (int) Math.round(lineColor.get().a * Math.min(1, alpha[0] / alpha[1]))),
                  shapeMode.get(),
                  0);
  ...
  private void onSend(PacketEvent.Send event) {
    if (mc.player != null && mc.level != null) {
      ...
      if (event.packet instanceof ServerboundUseItemOnPacket packet) {
        ...
        if (isOwn(packet.getHitResult().getBlockPos().above()))
          own.remove(packet.getHitResult().getBlockPos().above());
  ...
  private void update() {
    ...
    switch (switchMode.get()) {
      ...
      case Gapple -> {
        ...
        if (mc.options.keyUse.isDown()
            && Managers.HOLDING.isHolding(
                Items.END_CRYSTAL, Items.ENCHANTED_GOLDEN_APPLE, Items.GOLDEN_APPLE)
            && gapSlot >= 0) {
          if (getHand(OLEPOSSUtils::isGapple) == null) InvUtils.swap(gapSlot, false);
          handToUse = getHand(itemStack -> itemStack.getItem() == Items.END_CRYSTAL);
        } else if (Managers.HOLDING.isHolding(
            Items.END_CRYSTAL, Items.ENCHANTED_GOLDEN_APPLE, Items.GOLDEN_APPLE)) {
          int slot = InvUtils.findInHotbar(Items.END_CRYSTAL).slot();
          if (placePos != null && hand == null && slot >= 0) {
            InvUtils.swap(slot, false);
    ...
    if (placePos != null && placeDir != null) {
      if (!isPaused() && (!paPlace.get() || !Modules.get().isActive(PistonCrystal.class))) {
        ...
        if (handToUse != null
            || (switchMode.get() == SwitchMode.Silent && hotbar >= 0)
            || ((switchMode.get() == SwitchMode.PickSilent
                    || switchMode.get() == SwitchMode.InvSilent)
                && silentSlot >= 0)) {
          placing = true;
    ...
    if (!isPaused()
        && (hand != null
            || switchMode.get() == SwitchMode.Silent
            || switchMode.get() == SwitchMode.PickSilent
            || switchMode.get() == SwitchMode.InvSilent)
        && explode.get()) {
    ...
    if (expEntity != null) {
      if (multiTaskCheck()
          && !isAttacked(expEntity.getId())
          && attackDelayCheck()
          && existedCheck(expEntity.blockPosition())) {
  ...
  private boolean attackDelayCheck() {
    ...
    else return System.currentTimeMillis() > lastAttack + 1000 / expSpeed.get();
  }

  private boolean startAttackRot() {
    return (Managers.ROTATION.start(
        expEntity.getBoundingBox(),
        smartRot.get() ? expEntity.position() : null,
        priority + (!isAttacked(expEntity.getId()) && blocksPlacePos(expEntity) ? -0.1 : 0.1),
        ...
        && entity
            ...
            .intersects(
                new AABB(
                    ...
                    placePos.getZ(),
                    placePos.getX() + 1,
                    placePos.getY() + (SettingUtils.cc() ? 1 : 2),
                    placePos.getZ() + 1));
  }
  ...
  private void placeCrystal(
      BlockPos pos, Direction dir, InteractionHand handToUse, int sl, int hsl) {
  ...
  private boolean delayCheck() {
    if (placeDelayMode.get() == DelayMode.Seconds) return delayTimer >= placeDelay.get();
  ...
  private boolean isBlocked(BlockPos pos) {
    AABB box =
        new AABB(
            pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 2, pos.getZ() + 1);
    for (AABB bb : blocked) {
  ...
  private InteractionHand getHand(Predicate<ItemStack> predicate) {
    return predicate.test(Managers.HOLDING.getStack())
  ...
  private BlockPos getPlacePos() {
    ...
    int r =
        (int) Math.ceil(Math.max(SettingUtils.getPlaceRange(), SettingUtils.getPlaceWallsRange()));
    ...
    for (int x = -r; x <= r; x++) {
      for (int y = -r; y <= r; y++) {
        for (int z = -r; z <= r; z++) {
          ...
          if (!air(pos)
              || !(!SettingUtils.oldCrystals() || air(pos.above()))
          ...
          if (!inPlaceRange(pos.below())
              || !inExplodeRangePlacing(new Vec3(pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5)))
            continue;

          // Calculates damages and healths
          double[][] result =
              getDmg(new Vec3(pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5), false);
  ...
  private boolean explodeDamageCheck(double[] dmg, double[] health, boolean own) {
    boolean checkOwn =
        expMode.get() == ExplodeMode.FullCheck
  ...
  private double[][] getDmg(Vec3 vec, boolean attack) {
    ...
    for (Map.Entry<AbstractClientPlayer, AABB> entry : extPos.entrySet()) {
      ...
      double dmg =
          BODamageUtils.crystalDamage(player, box, vec, ignorePos(attack), ignoreTerrain.get());
    ...
    return new double[][] {
      new double[] {highestEnemy, highestFriend, self}, new double[] {enemyHP, friendHP}
    };
  }
  ...
  private boolean inExplodeRangePlacing(Vec3 vec) {
    return SettingUtils.inAttackRange(
        new AABB(vec.x() - 1, vec.y(), vec.z() - 1, vec.x() + 1, vec.y() + 2, vec.z() + 1),
  ...
  private boolean shouldSlow() {
    return placePos != null
        && getDmg(new Vec3(placePos.getX() + 0.5, placePos.getY(), placePos.getZ() + 0.5), false)[
                0][0]
            <= slowDamage.get();
  }
  ...
  private Vec3 smoothMove(Vec3 current, Vec3 target, double delta) {
    ...
    return new Vec3(
        current.x > target.x
            ? Math.max(target.x, current.x - x)
            : Math.min(target.x, current.x + x),
        current.y > target.y
            ? Math.max(target.y, current.y - y)
            : Math.min(target.y, current.y + y),
        current.z > target.z
            ? Math.max(target.z, current.z - z)
            : Math.min(target.z, current.z + z));
  }

  private boolean validForIntersect(Entity entity) {
    if (entity instanceof EndCrystal && canExplodePlacing(entity.position())) return false;
