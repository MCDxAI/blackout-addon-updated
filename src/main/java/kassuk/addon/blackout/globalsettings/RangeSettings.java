...
public class RangeSettings extends BlackOutModule {
  ...
  private final Setting<FromMode> placeRangeFrom =
      sgPlace.add(
          new EnumSetting.Builder<FromMode>()
  ...
  private final Setting<PlaceRangeMode> placeRangeMode =
      sgPlace.add(
          new EnumSetting.Builder<PlaceRangeMode>()
  ...
  public final Setting<Boolean> reduce =
      sgAttack.add(
          new BoolSetting.Builder()
              .name("Reduce")
              .description(
                  "Reduces range on every hit by reduce step until it reaches (range - reduce amount).")
              .defaultValue(false)
              .build());
  ...
  private final Setting<AttackRangeMode> attackRangeMode =
      sgAttack.add(
          new EnumSetting.Builder<AttackRangeMode>()
  ...
  private final Setting<MiningRangeMode> miningRangeMode =
      sgMining.add(
          new EnumSetting.Builder<MiningRangeMode>()
  ...
  public boolean inPlaceRange(BlockPos pos, Vec3 from) {
    ...
    return dist >= 0
        && dist <= (SettingUtils.placeTrace(pos) ? placeRange.get() : placeRangeWalls.get());
  }
      ...
      case CustomBox -> {
        return getRange(
            ...
            OLEPOSSUtils.getClosest(
                mc.player.getEyePosition(), feet, blockWidth.get(), blockHeight.get()));
      }
    }
    return -1;
  }
  ...
  public boolean inAttackRangeNoTrace(AABB bb, Vec3 feet, Vec3 from) {
    ...
    return attackRangeTo(bb, feet, from, true)
        <= Math.max(attackRange.get(), attackRangeWalls.get());
  }
  ...
  public double attackRangeTo(AABB bb, Vec3 feet, Vec3 from, boolean countReduce) {
    ...
    double dist =
        switch (attackRangeMode.get()) {
          ...
          case NCP ->
              getRange(
                  from, new Vec3(feet.x, Math.min(Math.max(from.y(), bb.minY), bb.maxY), feet.z));
          ...
          case UpdatedNCP ->
              getRange(
                      from,
                      new Vec3(feet.x, Math.min(Math.max(from.y(), bb.minY), bb.maxY), feet.z))
  ...
  public double getDistFromCenter(AABB bb, Vec3 feet, Vec3 from) {
    ...
    double angle =
        RotationUtils.radAngle(
            new Vec2((float) dist.x, (float) dist.z), new Vec2((float) vec2.x, (float) vec2.z));
  ...
  public boolean inMineRange(BlockPos pos) {
    ...
    return dist >= 0
        && dist <= (SettingUtils.placeTrace(pos) ? miningRange.get() : miningRangeWalls.get());
  }
