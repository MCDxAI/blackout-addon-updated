...
public class RotationSettings extends BlackOutModule {
  ...
  private final Setting<Boolean> interactRotate =
      rotateSetting("Interact", "interacting with a block", sgInteract);
  ...
  public final Setting<Double> interactYawAngle =
      yawAngleSetting("Interact", sgInteract, () -> interactMode.get() == RotationCheckMode.Angle);
  public final Setting<Double> interactPitchAngle =
      pitchAngleSetting(
          "Interact", sgInteract, () -> interactMode.get() == RotationCheckMode.Angle);
  public final Setting<Integer> interactMemory = memorySetting("Interact", sgInteract);

  // --------------------Block-Place--------------------//
  private final Setting<Boolean> blockRotate =
      rotateSetting("Block Place", "placing a block", sgBlockPlace);
  ...
  public final Setting<Double> blockYawAngle =
      yawAngleSetting(
          "Block Place", sgBlockPlace, () -> blockMode.get() == RotationCheckMode.Angle);
  ...
  public final Setting<MiningRotMode> mineTiming =
      sgMining.add(
          new EnumSetting.Builder<MiningRotMode>()
              .name("Mining Rotate Timing")
              .description(".")
              .defaultValue(MiningRotMode.End)
              .build());
  public final Setting<Double> mineYawAngle =
      yawAngleSetting("Mining", sgMining, () -> mineMode.get() == RotationCheckMode.Angle);
  public final Setting<Double> minePitchAngle =
      pitchAngleSetting("Mining", sgMining, () -> mineMode.get() == RotationCheckMode.Angle);
  public final Setting<Integer> mineMemory = memorySetting("Mining", sgMining);

  // --------------------Attack--------------------//
  private final Setting<Boolean> attackRotate =
      rotateSetting("Attack", "attacking an entity", sgAttack);
  ...
  public final Setting<Double> attackYawAngle =
      yawAngleSetting("Attack", sgAttack, () -> attackMode.get() == RotationCheckMode.Angle);
  public final Setting<Double> attackPitchAngle =
      pitchAngleSetting("Attack", sgAttack, () -> attackMode.get() == RotationCheckMode.Angle);
  ...
  private Setting<RotationCheckMode> modeSetting(String type, SettingGroup sg) {
    return sg.add(
        new EnumSetting.Builder<RotationCheckMode>()
            .name(type + " Rotation Mode")
            .description(".")
  ...
  private Setting<Double> yawAngleSetting(String type, SettingGroup sg, IVisible visible) {
    return sg.add(
        new DoubleSetting.Builder()
            .name(type + " Yaw Angle")
            .description("Accepts rotation if yaw angle to target is under this.")
            .defaultValue(90)
            .range(0, 180)
            .sliderRange(0, 180)
            .visible(visible)
  ...
  private Setting<Double> pitchAngleSetting(String type, SettingGroup sg, IVisible visible) {
    return sg.add(
        new DoubleSetting.Builder()
            .name(type + " Pitch Angle")
            .description("Accepts rotation if pitch angle to target is under this.")
            .defaultValue(45)
            .range(0, 180)
            .sliderRange(0, 180)
            .visible(visible)
      ...
      case StrictRaytrace -> {
        for (int r = 0; r < memory(type); r++) {
          ...
          Vec3 end =
              new Vec3(
                      range
                          * Math.cos(Math.toRadians(rot.yaw() + 90))
                          * Math.abs(Math.cos(Math.toRadians(rot.pitch()))),
                      range * -Math.sin(Math.toRadians(rot.pitch())),
                      range
                          * Math.sin(Math.toRadians(rot.yaw() + 90))
                          * Math.abs(Math.cos(Math.toRadians(rot.pitch()))))
                  .add(rot.vec());
  ...
  public boolean angleCheck(Vec3 pos, double y, double p, AABB box, RotationType type) {
    return RotationUtils.yawAngle(y, RotationUtils.getYaw(pos, box.getCenter())) <= yawAngle(type)
        && Math.abs(p - RotationUtils.getPitch(pos, box.getCenter())) <= pitchAngle(type);
  }
  ...
  public boolean raytraceCheck(Vec3 pos, double y, double p, AABB box) {
    ...
    Vec3 end =
        new Vec3(
                range * Math.cos(Math.toRadians(y + 90)) * Math.abs(Math.cos(Math.toRadians(p))),
                range * -Math.sin(Math.toRadians(p)),
                range * Math.sin(Math.toRadians(y + 90)) * Math.abs(Math.cos(Math.toRadians(p))))
            .add(pos);

    for (float i = 0; i < 1; i += 0.01) {
      if (box.contains(
          pos.x + (end.x - pos.x) * i, pos.y + (end.y - pos.y) * i, pos.z + (end.z - pos.z) * i))
        return true;
    }
    return false;
  }
