...
public class RotationManager {
          ...
          mc.getConnection()
              .send(
                  new ServerboundMovePlayerPacket.Rot(
                      next[0], next[1], Managers.ON_GROUND.isOnGround(), false));
        }
      }
    }
  }
  ...
  public ServerboundMovePlayerPacket onFull(ServerboundMovePlayerPacket.PosRot packet) {
    ...
    return new ServerboundMovePlayerPacket.Pos(
        packet.getX(0), packet.getY(0), packet.getZ(0), packet.isOnGround(), false);
  }
  ...
  private void updateNextRotation() {
    if (shouldRotate) {
      if (target instanceof BoxTarget) {
        ...
        next =
            new float[] {
              RotationUtils.nextYaw(
                  lastDir[0],
                  RotationUtils.getYaw(eyePos, ((BoxTarget) target).vec),
                  settings.yawStep(((BoxTarget) target).type)),
              RotationUtils.nextPitch(
                  lastDir[1],
                  RotationUtils.getPitch(eyePos, ((BoxTarget) target).vec),
        ...
        next =
            new float[] {
              RotationUtils.nextYaw(
                  lastDir[0],
                  ((AngleTarget) target).yaw,
                  settings.yawStep(((AngleTarget) target).type)),
              RotationUtils.nextPitch(
                  lastDir[1],
                  ((AngleTarget) target).pitch,
                  settings.pitchStep(((AngleTarget) target).type))
            };
      }

      rotated =
          Math.abs(RotationUtils.yawAngle(next[0], lastDir[0])) > 0
              || Math.abs(next[1] - lastDir[1]) > 0;
    }
  }
  ...
  public boolean start(BlockPos pos, AABB box, Vec3 vec, double p, RotationType type, long key) {
    ...
    if (p < priority
        || key == this.key
        || (p == priority
            && (!(target instanceof BoxTarget)
                || SettingUtils.rotationCheck(((BoxTarget) target).box, type)))) {
      if (!alreadyRotated) {
        priority = p;
      }
      ...
      target =
          pos != null
              ? new BoxTarget(pos, vec != null ? vec : OLEPOSSUtils.getMiddle(box), p, type)
  ...
  public Vec3 getTargetPos() {
    ...
    if (settings.mode(t.type) != RotationSettings.RotationCheckMode.StrictRaytrace
      ...
      return new Vec3(
          Mth.clamp(t.targetVec.x + (Math.random() - 0.5) * 0.05, t.box.minX, t.box.maxX),
          Mth.clamp(t.targetVec.y + (Math.random() - 0.5) * 0.05, t.box.minY, t.box.maxY),
          Mth.clamp(t.targetVec.z + (Math.random() - 0.5) * 0.05, t.box.minZ, t.box.maxZ));
    }
    ...
    for (double x = 0; x <= 1; x += 0.1) {
      for (double y = 0; y <= 1; y += 0.1) {
        for (double z = 0; z <= 1; z += 0.1) {
          Vec3 vec =
              new Vec3(
                  lerp(t.box.minX, t.box.maxX, x),
                  lerp(t.box.minY, t.box.maxY, y),
