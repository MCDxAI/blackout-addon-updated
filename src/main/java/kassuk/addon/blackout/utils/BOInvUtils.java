...
public class BOInvUtils {
  ...
  public static boolean pickSwitch(int slot) {
    if (slot >= 0) {
      ...
      HashedPatchMap.HashGenerator componentHasher =
          ((ComponentHasherNetworkHandlerAccessor) mc.getConnection())
              .getDecoratedHashOpsGenerator();
      HashedStack stackHash = HashedStack.create(stack, componentHasher);

      ServerboundContainerClickPacket packet =
          new ServerboundContainerClickPacket(
              syncId, revision, (short) slot, (byte) button, actionType, modifiedStacks, stackHash);
      ...
      mc.getConnection()
          .send(
              new ServerboundContainerClickPacket(
                  handler.containerId,
                  handler.getStateId(),
                  (short) (Inventory.INVENTORY_SIZE + Managers.HOLDING.slot),
                  (byte) slot,
                  ContainerInput.SWAP,
                  modifiedStacks,
                  stackHash));
  ...
  public static void swapBack() {
    ...
    ItemStack stack = handler.getSlot(slot).getItem();
    HashedPatchMap.HashGenerator componentHasher =
        ((ComponentHasherNetworkHandlerAccessor) mc.getConnection()).getDecoratedHashOpsGenerator();
    HashedStack stackHash = HashedStack.create(stack, componentHasher);
    Int2ObjectMap<HashedStack> modifiedStacks = new Int2ObjectOpenHashMap<>();
    ...
    mc.getConnection()
        .send(
            new ServerboundContainerClickPacket(
                handler.containerId,
                handler.getStateId(),
                (short) (Inventory.INVENTORY_SIZE + slots[1]),
                (byte) slots[0],
                ContainerInput.SWAP,
                modifiedStacks,
                stackHash));
