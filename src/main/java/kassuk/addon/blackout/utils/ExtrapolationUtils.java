...
public class ExtrapolationUtils {
  ...
  public static void extrapolateMap(
      Map<AbstractClientPlayer, AABB> old,
      EpicInterface<AbstractClientPlayer, Integer> extrapolation,
      EpicInterface<AbstractClientPlayer, Integer> smoothening) {
    old.clear();
    ...
    motions.forEach(
        (player, m) -> {
          if (m == null) return;
          old.put(
              player, extrapolate(player, m, extrapolation.get(player), smoothening.get(player)));
        });
  }
  ...
  public static AABB extrapolate(
      AbstractClientPlayer player, List<Vec3> m, int extrapolation, int smoothening) {
    ...
    AABB box =
        new AABB(
            ...
            player.getZ() - 0.3,
            player.getX() + 0.3,
            player.getY() + (player.getBoundingBox().maxY - player.getBoundingBox().minY),
            player.getZ() + 0.3);
    ...
    for (int i = 0; i < extrapolation; i++) {
      ...
      Vec3 vec3d =
          movement.lengthSqr() == 0.0
              ? movement
              : Entity.collideBoundingBox(player, movement, box, mc.level, list);

      boolean canStep = (onGround || (y < 0 && vec3d.y != y)) && (vec3d.x != x || vec3d.z != z);

      if (canStep) {
        Vec3 vec3d2 =
            Entity.collideBoundingBox(player, new Vec3(x, stepHeight, z), box, mc.level, list);
        Vec3 vec3d3 =
            Entity.collideBoundingBox(
                player,
                new Vec3(0.0, stepHeight, 0.0),
                box.expandTowards(x, 0.0, z),
                mc.level,
                list);
        ...
        if (vec3d2.horizontalDistanceSqr() > vec3d.horizontalDistanceSqr()) {
          Vec3 vec =
              vec3d2.add(
                  Entity.collideBoundingBox(
                      player,
                      new Vec3(0.0, -vec3d2.y + movement.y, 0.0),
                      box.move(vec3d2),
                      mc.level,
                      list));
          box = box.move(vec);
