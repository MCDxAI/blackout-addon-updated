...
public class BlackOutModule extends Module {
  private static final Component PREFIX =
      Component.literal("[BlackOut] ").withStyle(ChatFormatting.DARK_RED);
  ...
  public void sendBOInfo(String text) {
    if (mc.level != null) {
      ChatUtils.forceNextPrefixClass(getClass());
      Component msg =
          Component.empty().append(PREFIX).append(name).append(CommonComponents.SPACE).append(text);
  ...
  public void sendSequenced(PredictiveAction packetCreator) {
    ...
    BlockStatePredictionHandler sequence =
        mc.level.getBlockStatePredictionHandler().startPredicting();
  ...
  public void placeBlock(
      InteractionHand hand, Vec3 blockHitVec, Direction blockDirection, BlockPos pos) {
    ...
    sendSequenced(
        s ->
            new ServerboundUseItemOnPacket(
                hand, new BlockHitResult(blockHitVec, blockDirection, pos, inside), s));
    ...
    sendSequenced(
        s ->
            new ServerboundUseItemPacket(
                hand, s, Managers.ROTATION.lastDir[0], Managers.ROTATION.lastDir[1]));
