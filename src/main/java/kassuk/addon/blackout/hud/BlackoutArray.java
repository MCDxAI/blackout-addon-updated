...
public class BlackoutArray extends HudElement {
  ...
  private final Setting<Boolean> infoCare =
      sgGeneral.add(
          new BoolSetting.Builder()
              ...
              .defaultValue(true)
              .build());
  private final Setting<Boolean> onlyBlackout =
      sgGeneral.add(
          new BoolSetting.Builder()
              .name("Only Blackout")
  ...
  public static final HudElementInfo<BlackoutArray> INFO =
      new HudElementInfo<>(
          BlackOut.HUD_BLACKOUT,
          "BlackoutArray",
          "An ArrayList for blackout features.",
          BlackoutArray::new);
  ...
  public void render(HudRenderer renderer) {
                ...
                Comparator.comparing(
                    module ->
                        width(
                            ...
                            module.title
                                + (infoCare.get()
                                    ? (getInfo(module).isEmpty() ? "" : getInfo(module) + " ")
                                    : ""))))
    ...
    setSize(
        120 * scale.get() * scale.get(),
        !lines.isEmpty() ? height * lines.size() : 30 * scale.get() * scale.get());
    ...
    for (int i = 0; i < lines.size(); i++) {
      ...
      renderer.text(
          ...
          side.get() == Side.Left
              ...
              : x
                  + getWidth()
                  - width(renderer, line.name + (line.info.isEmpty() ? "" : " " + line.info)),
          y + i * height,
          getColor(color.get(), waveColor.get(), f),
          shadow.get(),
          scale.get());
      renderer.text(
          line.info,
          side.get() == Side.Left
              ? x + width(renderer, line.name + " ")
              : x + getWidth() - width(renderer, line.info),
          y + i * height,
          getColor(infoColor.get(), infoWaveColor.get(), f),
          shadow.get(),
          scale.get());
    }
  }
  ...
  private Color getColor(SettingColor color, SettingColor waveColor, double f) {
    return wave.get()
        ? new Color(
            colorVal(color.r, waveColor.r, f),
            colorVal(color.g, waveColor.g, f),
            colorVal(color.b, waveColor.b, f),
            color.a)
        : color;
  }
