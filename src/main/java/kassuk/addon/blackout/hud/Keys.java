...
public class Keys extends HudElement {
  ...
  public static final HudElementInfo<Keys> INFO =
      new HudElementInfo<>(
          BlackOut.HUD_BLACKOUT, "Keys", "Draws pressed movement keys.", Keys::new);
  ...
  public void render(HudRenderer renderer) {
    if (keys == null) {
      ...
      KeyMapping[] binds =
          new KeyMapping[] {
            mc.options.keyUp, mc.options.keyLeft, mc.options.keyDown, mc.options.keyRight
          };
      for (int i = 0; i < 4; i++) {
    ...
    keys.forEach(
        key -> {
          ...
          if (textBG.get()) {
            renderer.quad(
                key.posX + 2 * scale.get() * scale.get(),
                key.posY + 2 * scale.get() * scale.get(),
                36 * scale.get() * scale.get(),
                36 * scale.get() * scale.get(),
                getBGColor(key));
          }
          renderer.text(
              key.key,
              key.posX + xOffset(key.key, renderer),
              key.posY + yOffset(renderer),
              getTextColor(key),
              false,
              scale.get());
        });
  ...
  private Color getBGColor(Key k) {
    return lerpColor(
        Mth.clamp((k.sinceClick() - renderTime.get() * 1000) / fadeTime.get() / 1000, 0, 1),
        cbgColor.get(),
        bgColor.get());
  }
  ...
  private Color lerpColor(double delta, Color s, Color e) {
    return new Color(
        (int) Math.round(Mth.lerp(delta, s.r, e.r)),
        (int) Math.round(Mth.lerp(delta, s.g, e.g)),
        (int) Math.round(Mth.lerp(delta, s.b, e.b)),
        (int) Math.round(Mth.lerp(delta, s.a, e.a)));
  }
